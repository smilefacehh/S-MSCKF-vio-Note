...
bool ImageProcessor::loadParameters() {
  ...
  // 相机与IMU外参，注：本代码中的坐标关系与常规表示方法相反，比如T_imu_cam0是从imu系转换到cam0系，常规表示是Tci
 ...
 * 1、创建光流金字塔
 * 2、第一帧,双目初始化,提取特征点
 * 3、特征点跟踪，添加跟踪点到curr_features_ptr
 * 4、添加当前帧新特征点到curr_features_ptr
 * 5、删除多余特征点
 * 6、发布当前帧特征点，跟踪点数量信息
*/
void ImageProcessor::stereoCallback(
  ...
  if (is_first_img) {
     ...
     * 1、提取左目Fast特征点
     * 2、双目匹配，得到右目特征点
     * 3、划分网格，每个网格中的点按照响应值从大到小排序
     * 4、每个网格，添加最少的优质点到curr_features_ptr，作为当前帧特征点
    */
    ...
    /**
     * 特征点跟踪
     * 1、用前一帧与当前帧之间的IMU角速度平均值，乘上时间dt，得到旋转增量R
     * 2、前一帧特征点，通过旋转R，变换得到当前帧特征点。只有旋转，这是一个初始估计
     * 3、LK光流跟踪前后帧的左目特征点，剔除outlier点
     * 4、当前帧双目匹配，进一步剔除outlier点
     * 5、最后，前后帧的左目-左目，右目-右目通过对极约束(Rp2 x p1)·t=0，ransac剔除outlier点，并计算最优t
     * 6、最终剩余的特征点加入curr_features_ptr
    */
    ros::Time start_time = ros::Time::now();
    trackFeatures();
    ...
    /**
     * 添加当前帧新特征点到curr_features_ptr
     * 1、跟踪点附近区域设置mask，避开，提取fast特征点
     * 2、当前帧双目匹配，剔除outlier点
     * 3、补齐curr_features_ptr到最少点标准
    */
    start_time = ros::Time::now();
    addNewFeatures();
...
void ImageProcessor::initializeFirstFrame() {
   ...
   * 双目匹配
   * 1、左目点通过外参变换得到右目点
   * 2、LK光流跟踪匹配点，剔除outlier点
   * 3、剔除图像区域外的点
   * 4、外参构造本质矩阵E，计算点到极线距离误差，剔除outlier点
  */
  ...
  // 匹配点存入map中，<网格id，vector<FeatureMetaData>>
  ...
  for (int code = 0; code <
    ...
    // 把当前帧该网格中的特征点加到curr_features_ptr中
    // 注：这里应该是grid_max_feature_num
    for (int k = 0; k < processor_config.grid_max_feature_num &&
...
/**
 * 前一帧特征点，通过旋转R，变换得到当前帧特征点。只有旋转，这是一个初始估计
*/
...
void ImageProcessor::trackFeatures() {
  ...
  Matx33f cam0_R_p_c;
  Matx33f cam1_R_p_c;
  // 用前一帧与当前帧之间的IMU角速度平均值，乘上时间dt，得到旋转增量
  ...
  // LK光流跟踪前后帧左目特征点，剔除outlier点，剩下的匹配点集合
  // 前一帧左、右目，当前帧左目
  ...
  */
  ...
  // 对前后帧左目-左目，右目-右目，执行ransac求解平移t
  /**
   * 已知两帧匹配点，旋转R，ransac求解平移t
   * 通过匹配点可以直接计算E、F，恢复R、t（多解），这里通过IMU已知了旋转R，通过对极约束可以求t，ransac得出内点最多对应的t
   * 1、通过纯旋转，计算误差，超过50像素（很宽松）认为是outlier点，如果inlier点数不足3个，返回
   * 2、如果纯旋转计算的平均误差小于1个像素，认为没有平移，对极约束不成立，返回
   * 3、归一化相机平面上，点坐标缩放到距离中心点根号2附近
   * 4、利用对极约束(Rp2 x p1)·t=0，取2个点计算t，计算该t下的inlier点数，执行多次，记录inlier点数最多的那次
  */
  ...
  // 提取最终的inlier匹配点，加入curr_features_ptr，更新lifetime
  after_ransac = 0;
...
void ImageProcessor::stereoMatch(
  ...
  // 如果cam1没有提取特征点，直接用cam0的特征点+外参，变换到cam1下
  if(cam1_points.size() == 0) {
    ...
    // cam0的特征点（像素坐标），畸变校正，再投影到cam1系下，得到cam1归一化相机点
  ...
  for (int i = 0; i < cam0_points_undistorted.size(); ++i) {
    ...
    if (error > processor_config.stereo_threshold*norm_pixel_unit)
      inlier_markers[i] = 0;
    printf("[LT][stereoMatch] 极线约束，error=%f, th=%f", error, processor_config.stereo_threshold*norm_pixel_unit);
  }
...
void ImageProcessor::addNewFeatures() {
  ...
  // 对每个网格删除超出数量限制的特征点存new_features
  new_features.clear();
...
void ImageProcessor::undistortPoints(
  ...
  if (distortion_model == "radtan") {
    // pts_in通过K变换到相机坐标系，然后畸变校正一下，再通过rectification_matrix变换到右目相机坐标系，再通过K_new变换为右目像素坐标
    // 如果K_new是单位矩阵，那么结果就是归一化相机点
...
vector<cv::Point2f> ImageProcessor::distortPoints(
  ...
  if (distortion_model == "radtan") {
    ...
    // homogenous_pts通过R、t转换到相机系，归一化，畸变校正，再通过K转换成像素点。因为R、t、K都是单位矩阵或0，homogenous_pts等同像素点
...
void ImageProcessor::rescalePoints(
  ...
  // p/d*sqrt(2)，d是平均距离，相当于将点坐标缩放到距离中心点根号2附近
  for (int i = 0; i < pts1.size(); ++i) {
...
void ImageProcessor::twoPointRansac(
  ...
  // 归一化相机平面上对应1个像素的单位距离, 1/f*u = P
  double norm_pixel_unit = 2.0 / (intrinsics[0]+intrinsics[1]);
  // ransac每次采样2个点，需要的迭代次数，0.7是outlier概率
  ...
  // 如果平均误差小于1个像素，认为运动只有旋转，没有平移，极线约束不成立
  // 直接返回，剔除误差超过3个像素的点
  ...
  // 迭代ransac，利用极线约束方程p1*txR*p2=0, 变换一下(Rp2 x p1)·t=0, 共面关系，t尺度不确定，固定一个元素为1，只需要 2个点，就能求解t
  for (int iter_idx = 0; iter_idx < iter_num; ++iter_idx) {
    ...
    if (base_indicator == 0) {
      ...
      // 按列堆积
      A << coeff_ty, coeff_tz;
      // 直接求解(Rp2 x p1)·t中的t，即为这里的solution
    ...
    // 将误差小于3个像素的点，加入当前ransac迭代的inlier点集合中
