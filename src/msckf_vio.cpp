...
/**
 * 订阅IMU
 * 前200帧用于初始化角速度偏置，IMU系与world系的旋转
*/
...
void MsckfVio::featureCallback(
  ...
  // 状态传播，前一帧与当前帧之间的IMU数据，计算pvq，更新协方差矩阵P
  ...
  */
  ...
  /**
   * 对当前帧跟丢的点，构建误差模型，观测更新，从滑窗集合中删除这些点
   * 计算卡尔曼增益，误差状态，更新IMU状态，更新所有相机状态，更新系统协方差矩阵
  */
  start_time = ros::Time::now();
  removeLostFeatures();
 ...
 * 1、IMU状态传播，更新q,v,p
 *   1) 毕卡法更新四元数
 *   2) 4阶龙格库塔（Runge-Kutta）积分更新速度、位置
 * 2、更新系统协方差矩阵P
 * @param time_bound 当前图像帧时间戳
*/
...
void MsckfVio::processModel(const double& time,
  ...
  // IMU状态x=(q,bg,v,ba,p,qe,pe) 23维，最后2个是相机与imu外参
  // IMU误差状态dx=(dq,dbg,dv,dba,dp,dqe,dpe) 21维
  // IMU误差状态连续时间模型，D是导数，dx是状态量误差，n是噪声
  // D(dx) = F·dx + G·n
...
void MsckfVio::predictNewState(const double& dt,
  ...
  // 4阶龙格库塔（Runge-Kutta）积分求解速度、位置；vins-mono中使用中值积分，另外还有常见的欧拉积分
  // k1 = f(tn, yn)
...
void MsckfVio::removeLostFeatures() {
  ...
  // 如果没有跟丢的特征点，直接返回
  // 为什么只更新跟丢的特征点，因为希望特征点有尽可能多的观测之后，优化重投影误差得到的结果会更加准确
  // 只要特征点还在被观测到,就不进行观测更新,让它加入更多的观测,后面在某帧不再被观测到的时候,再进行观测更新
  if (processed_feature_ids.size() == 0) return;

  // 构建重投影误差模型的近似Hessian和残差
